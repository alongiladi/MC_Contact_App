...
import at.uastw.contactsapp.ui.theme.Typography

enum class Routes (val route: String){
    List("list"),
    // contactId is path parameter (placeholder) for navigation
...
fun ContactsApp(modifier: Modifier = Modifier,
    ...
    // display the current screen depending on the navigation state
    NavHost(
        navController, modifier = modifier, startDestination = Routes.List.route
    ){
...
fun ContactsListView(modifier: Modifier = Modifier,
    ...
    // Log messages help us with debugging the app, they are shown in the "Logcat" view
...
fun ContactDetailView(contactDetailViewModel: ContactDetailViewModel = viewModel(factory = AppViewModelProvider.Factory)){
    ...
    // to read the contact that a user click
